...
# 表格从0开始计数，2:3 是提取[2:3)列，前闭后开,故提取出C列开盘价
...
# 测试集：csv表格中后300天数据
# 利用for循环，遍历整个测试集，
# 提取测试集中连续60天的开盘价作为输入特征x_train，第61天的数据作为标签，
# for循环共构建300-60=240组数据。
for i in range(60,len(test_set)):
    x_test.append(test_set[i-60:i,0])
    y_test.append(test_set[i,0])
# 测试集变array 并 reshape为符合RNN输入要求：
...
# 损失函数用均方误差
# 该应用只观测loss数值，不观测准确率，
# 所以删去metrics选项，一会在每个epoch迭代显示时只显示loss值
...
plt.plot(predicted_stock_price,color='blue',label='Perdicted MaoTai Stock Price')
...
#评估
# calculate MSE 均方误差 --->
# E[(预测值-真实值)^2] (预测值减真实值求平方后求均值)
mse=mean_squared_error(predicted_stock_price,real_stock_price)
# calculate RMSE 均方根误差--->sqrt[MSE]    (对均方误差开方)
rmse=math.sqrt(mean_squared_error(predicted_stock_price,real_stock_price))
# calculate MAE 平均绝对误差----->
# E[|预测值-真实值|](预测值减真实值求绝对值后求均值）
