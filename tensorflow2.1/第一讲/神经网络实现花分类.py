...
#seed随机数种子，是一个整数，当设置之后，每次生成的随机数都一样，（每个人结果都一样）
...
x_test=x_data[-30:]
y_test=y_data[-30:]
#转换x的数据类型，保持数据类型一致，否则后面矩阵相乘时会因数据类型不一致报错
x_train=tf.cast(x_train,tf.float32)
x_test=tf.cast(x_test,tf.float32)
#from_tensor_slices函数使输入特征和标签值一一对应。数据集分批次，每个批次batch组数据
train_db=tf.data.Dataset.from_tensor_slices((x_train,y_train)).batch(32)
test_db=tf.data.Dataset.from_tensor_slices((x_test,y_test)).batch(32)
#生成神经网络的参数，4个输入特征，故输入层为4个输入节点，因为3分类，故输出层为3个神经元
#tf.Variable()标记参数可训练
#seed使每次生成的随机数相同，现实中使用不写seed
w1=tf.Variable(tf.random.truncated_normal([4,3],stddev=0.1,seed=1))
b1=tf.Variable(tf.random.truncated_normal([3],stddev=0.1,seed=1))

lr=0.1
train_loss_results=[] #将每轮的loss记录在次列表中，为后续画loss曲线提供数据
test_acc=[]#将每轮的acc记录在此列表中，为后续画acc曲线提供数据
epoch=500#循环500轮
loss_all=0#每轮4个step，loss_all记录四个step生成的4个loss的和
#训练部分
